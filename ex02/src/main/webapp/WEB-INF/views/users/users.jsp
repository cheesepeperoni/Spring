...
<script type="text/javascript">
	...
	function userSelect() {
		...
		$('body').on('click', '#btnSelect', function() {
			//var userId = $(this).closest('tr').find('#hidden_userId').val();  				방법1
			//var userId = $(this).parent().parent().data("userid");			  	 		 //방법2
			var userId = $(this).closest('tr').data("userid"); // 제일 가까운 tr 태그를 읽어옴	    방법3
			var tr = $(this).closest('tr');
			//특정 사용자 조회
			$.ajax({
				url : 'users/' + userId,
	...
	function userList() {
		$.ajax({
			url : 'users',
			type : 'GET',
			//contentType:'application/json;charset=utf-8',	get방식이니까 필요없음
			dataType : 'json',
			error : function(xhr, status, msg) {
				alert("상태값 :" + status + " Http에러메시지 :" + msg);
			},
	...
	function userListResult(data) {
		...
		$.each(data, function(idx, item) { // for(i=0; i<data.length; i++)  => item=data[i]
			$('<tr data-userid="'+item.id+'">').append($('<td>').html(item.id))
					.append($('<td>').html(item.name)).append(
...
<div class="row">
	<div class="col-lg-12">
		<h1 class="page-header" style="font-weight: bold;">사용자 관리</h1>
	</div>
